...
} )( typeof window === "undefined" ? this : window, ( window, noGlobal ) => {
...
// unguarded in another place, it seems safer to define global only for this module
    ...
    jQuery.extend = jQuery.fn.extend = function() {
        let clone; let copy; let copyIsArray; let deep = false; let i = 1; const length = arguments.length;
            let name;
            let options;
            let src;
 ...
 */
        ( function( window ) {
            let arr = [];
                ...
                const whitespace = String.raw`[\x20\t\r\n\f]`;
                // https://www.w3.org/TR/css-syntax-3/#ident-token-diagram
                const identifier = `${String.raw`(?:\\[\da-fA-F]{1,6}` + whitespace 
                    }?|\\\\[^\\r\\n\\f]|[\\w-]|[^\0-\\x7f])+`;
                // Attribute selectors: http://www.w3.org/TR/selectors/#attribute-selectors
                const attributes = `${String.raw`\[` + whitespace  }*(${  identifier  })(?:${  whitespace 
                ...
                const expando = `sizzle${  1 * Date.now()}`;
                let Expr;
                ...
                const funescape = function( escape, nonHex ) {
                    const high = `0x${  escape.slice( 1 )}` - 0x1_00_00;
                ...
                const runescape = new RegExp( String.raw`\\[\da-fA-F]{1,6}` + whitespace + String.raw`?|\\([^\r\n\f])`, "g" );
...
// Optimize for push.apply( _, NodeList )
            ...
            function Sizzle( selector, context, results, seed ) {
                let elem; let groups; let i; let m; let match; let newContext = context && context.ownerDocument; let newSelector;
                    let nid;
...
// Expose support vars for convenience
            ...
            setDocument = Sizzle.setDocument = function( node ) {
                const doc = node ? node.ownerDocument || node : preferredDoc; let hasCompare;
                    let subWindow;
                ...
                support.scope = assert( ( el ) => {
                    docElem.appendChild( el ).append( document.createElement( "div" ) );
	...
	---------------------------------------------------------------------- */
                ...
                support.getElementsByTagName = assert( ( el ) => {
                    el.append( document.createComment( "" ) );
                ...
                if ( support.getById ) {
                    Expr.filter.ID = function( id ) {
                        const attrId = id.replaceAll( runescape, funescape );
                        return function( elem ) {
                    ...
                    Expr.filter.ID =  function( id ) {
                        const attrId = id.replaceAll( runescape, funescape );
                        return function( elem ) {
                            const node = elem.getAttributeNode !== undefined &&
                    ...
                    Expr.find.ID = function( id, context ) {
                        if ( context.getElementById !== undefined && documentIsHTML ) {
                            let elem = context.getElementById( id ); let elems; let i;
                                let node;
			...
			*/
                ...
                filter: {
                    ...
                    "TAG": function( nodeNameSelector ) {
                        const nodeName = nodeNameSelector.replaceAll( runescape, funescape ).toLowerCase();
                                    ...
                                    operator === "^=" ? check && result.indexOf( check ) === 0 :
                                        operator === "*=" ? check && result.includes(check) :
                                            operator === "$=" ? check && result.slice( -check.length ) === check :
                                                operator === "~=" ? ( ` ${  result.replaceAll( rwhitespace, " " )  } ` ).includes(check) :
                        ...
                        return first === 1 && last === 0 ?
                            ...
                            function( elem, _context, xml ) {
                                let cache; let uniqueCache; let outerCache; let node; let nodeIndex; let start;
                                    let dir = simple === forward ? "previousSibling" : "nextSibling";
                ...
                pseudos: {
                    ...
                    "not": markFunction( ( selector ) => {
                        ...
                        const input = [];
                            const results = [];
                            const matcher = compile( selector.replaceAll( rtrim, "$1" ) );
...
// Easy API for creating new setFilters
            ...
            tokenize = Sizzle.tokenize = function( selector, parseOnly ) {
                const cached = tokenCache[ `${selector  } ` ]; let groups; let match; let matched;
                    let preFilters; let soFar; let tokens;
                    let type;
            ...
            function addCombinator( matcher, combinator, base ) {
                ...
                return combinator.first ?
                    ...
                    function( elem, context, xml ) {
                        const newCache = [ dirruns, doneName ]; let oldCache; let outerCache;
                            let uniqueCache;
            ...
            function setMatcher( preFilter, selector, matcher, postFilter, postFinder, postSelector ) {
                ...
                return markFunction( ( seed, results, context, xml ) => {
                    let elem; // Get initial elements from seed or context
            ...
            function matcherFromTokens( tokens ) {
                let checkContext; let matcher; let j;
                    ...
                    const matchContext = addCombinator( ( elem ) => {
            ...
            function matcherFromGroupMatchers( elementMatchers, setMatchers ) {
                const byElement = elementMatchers.length > 0;
                    ...
                    const superMatcher = function( seed, context, xml, results, outermost ) {
                        const contextBackup = outermostContext; // Use integer dirruns iff this is the outermost matcher
                            const dirrunsUnique = ( dirruns += contextBackup == null ? 1 : Math.random() || 0.1 ); let elem;
            ...
            select = Sizzle.select = function( selector, context, results, seed ) {
                const compiled = typeof selector === "function" && selector; let find; let i; const match = !seed && tokenize( ( selector = compiled.selector || selector ) ); let token;
                    let tokens;
                    let type;
...
//	6. Provide a clear path for implementation upgrade to WeakMap in 2014
    ...
    jQuery.fn.extend( {
        data( key, value ) {
            const elem = this[ 0 ]; const attrs = elem && elem.attributes; let data;
                let i;
                let name;
...
// Mark scripts as having already been evaluated
    ...
    function buildFragment( elems, context, scripts, selection, ignored ) {
        let attached; let elem; const fragment = context.createDocumentFragment(); let i = 0; let j; const l = elems.length;
            const nodes = [];
            let tag;
            let tmp;
            let wrap;
 ...
 */
    jQuery.event = {
        ...
        add( elem, types, handler, data, selector ) {
            ...
            const elemData = dataPriv.get( elem ); let eventHandle; let events;
                let handleObj; let handleObjIn; let handlers;
                let namespaces; let origType; let special; let t; let tmp;
                let type;
        ...
        remove( elem, types, handler, selector, mappedTypes ) {
            ...
            const elemData = dataPriv.hasData( elem ) && dataPriv.get( elem ); let events; let handleObj;
                let handlers; let j; let namespaces;
                let origCount; let origType; let special; let t; let tmp;
                let type;
        ...
        dispatch( nativeEvent ) {
            ...
            const args = Array.from({length: arguments.length}); // Make a writable jQuery.Event from the native event object
                const event = jQuery.event.fix( nativeEvent ); let handleObj; let handlerQueue; const handlers = (
        ...
        handlers( event, handlers ) {
            let cur = event.target; const delegateCount = handlers.delegateCount; let handleObj; const handlerQueue = []; let i;
                let matchedHandlers;
...
// Replace/restore the type attribute of script elements for safe DOM manipulation
    ...
    function cloneCopyEvent( src, dest ) {
        let events, i, l, pdataOld, type, udataCur, udataOld;
...
// Fix IE bugs, see support tests
    ...
    function domManip( collection, args, callback, ignored ) {
        ...
        let doc; let first; let fragment; let hasScripts; let i = 0; const l = collection.length;
            const iNoClone = l - 1;
            let node;
            let scripts;
            const value = args[ 0 ];
    ...
    jQuery.extend( {
        ...
        clone( elem, dataAndEvents, deepDataAndEvents ) {
            const clone = elem.cloneNode( true ); let destElements; let i; const inPage = isAttached( elem );
                let l;
                let srcElements;
    ...
    ( function() {
        ...
        function computeStyleTests() {
            ...
            div.style.cssText =
                ...
                "width:60%;top:1%";
            documentElement.appendChild( container ).append( div );
        ...
        let boxSizingReliableVal; var container = document.createElement( "div" ); var div = document.createElement( "div" ); let pixelBoxStylesVal;
            let pixelPositionVal; let reliableMarginLeftVal;
        ...
        jQuery.extend( support, {
            ...
            reliableTrDimensions() {
                ...
                if ( reliableTrDimensionsVal == null ) {
                    ...
                    reliableTrDimensionsVal = ( Number.parseInt( trStyle.height, 10 ) +
                        Number.parseInt( trStyle.borderTopWidth, 10 ) +
                        Number.parseInt( trStyle.borderBottomWidth, 10 ) ) === tr.offsetHeight;
    ...
    function curCSS( elem, name, computed ) {
        const isCustomProp = rcustomProp.test( name ); let maxWidth; let minWidth; let ret;
            // Support: Firefox 51+
...
// Return a potentially-mapped jQuery.cssProps or vendor prefixed property
    ...
    jQuery.extend( {
        ...
        style( elem, name, value, extra ) {
            ...
            let hooks; const isCustomProp = rcustomProp.test( name ); const origName = camelCase( name );
                let ret;
                const style = elem.style;
                let type;
        ...
        css( elem, name, extra, styles ) {
            let hooks; const isCustomProp = rcustomProp.test( name ); let num;
    ...
    jQuery.cssHooks.marginLeft = addGetHookIf( support.reliableMarginLeft,
        ( elem, computed ) => {
            if ( computed ) {
                return `${Number.parseFloat( curCSS( elem, "marginLeft" ) ) ||
...
// These hooks are used by animate to expand properties
    ...
    Tween.prototype = {
        ...
        run( percent ) {
            ...
            this.pos = eased = this.options.duration ? jQuery.easing[ this.easing ](
...
// Generate parameters to create a standard animation
    ...
    function defaultPrefilter( elem, props, opts ) {
        const anim = this; let dataShow = dataPriv.get( elem, "fxshow" ); let display; let hidden = elem.nodeType && isHiddenWithinTree( elem ); let hooks; const isBox = "width" in props || "height" in props; let oldfire; const orig = {};
            let prop;
            let propTween;
            let restoreDisplay;
            const style = elem.style;
...
// Based off of the plugin by Clint Helfers, with permission.
    ...
    jQuery.extend( {
        attr( elem, name, value ) {
            let hooks; const nType = elem.nodeType;
                let ret;
...
// Hooks for boolean attributes
    ...
    jQuery.each( jQuery.expr.match.bool.source.match( /\w+/g ), ( _i, name ) => {
        ...
        attrHandle[ name ] = function( elem, name, isXML ) {
            let handle; const lowercaseName = name.toLowerCase();
                let ret;
...
// since it considers such accessions noop
    ...
    jQuery.fn.extend( {
        addClass( value ) {
            let className, classNames, cur, curValue, finalValue, i;
        ...
        toggleClass( value, stateVal ) {
            let className; let classNames; let i; const type = typeof value;
    ...
    jQuery.fn.extend( {
        val( value ) {
            const elem = this[ 0 ]; let hooks; let ret;
                let valueIsFunction;
    ...
    jQuery.extend( {
        valHooks: {
            ...
            select: {
                get( elem ) {
                    let i; const index = elem.selectedIndex; const one = elem.type === "select-one";
                ...
                set( elem, value ) {
                    const options = elem.options; let i = options.length;
                        let option;
                        let optionSet;
...
// Return jQuery for attributes-only inclusion
    ...
    jQuery.extend( jQuery.event, {
        ...
        trigger( event, data, elem, onlyHandlers ) {
            ...
            let bubbleType; let cur; const eventPath = [ elem || document ]; let handle; let i; let lastElement; let namespaces = hasOwn.call( event, "namespace" ) ? event.namespace.split( "." ) : []; let ontype;
                let special;
                let tmp;
...
// Cross-browser xml parsing
    jQuery.parseXML = function( data ) {
        ...
        parserErrorElem = xml && xml.querySelectorAll( "parsererror" )[ 0 ];
        if ( !xml || parserErrorElem ) {
            jQuery.error( `Invalid XML: ${  
                parserErrorElem ?
                    jQuery.map( parserErrorElem.childNodes, ( el ) => {
...
// Fixes trac-9887
    function ajaxExtend( target, src ) {
        let deep; const flatOptions = jQuery.ajaxSettings.flatOptions || {};
            let key;
 ...
 */
    function ajaxHandleResponses( s, jqXHR, responses ) {
        ...
        const contents = s.contents; let ct; const dataTypes = s.dataTypes; let finalDataType;
            let firstDataType;
            let type;
 ...
 */
    function ajaxConvert( s, response, jqXHR, isSuccess ) {
        let conv; let conv2; const converters = {}; let current; // Work with a copy of dataTypes in case we need to modify it for conversion
		...
		*/
        ...
        ajax( url, options ) {
            ...
            function done( status, nativeStatusText, responses, headers ) {
                let error; let isSuccess; let modified; let response; let statusText = nativeStatusText;
                    let success;
...
// keepScripts (optional): If true, will include scripts passed in the html string
    ...
    jQuery.fn.load = function( url, params, callback ) {
        const off = url.indexOf( " " ); let response; let selector;
            const self = this;
            let type;
    ...
    jQuery.offset = {
        setOffset( elem, options, i ) {
            let calculatePosition; let curCSSLeft; let curCSSTop; const curElem = jQuery( elem ); let curLeft; let curOffset; let curPosition;
                let curTop;
    ...
    jQuery.fn.extend( {
        ...
        position() {
            ...
            let doc; const elem = this[ 0 ]; let offset;
                let offsetParent;
