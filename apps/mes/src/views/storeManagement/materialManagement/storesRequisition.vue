...
const gridOptions: VxeGridProps<any> = {
  ...
  columns: [
    ...
    { field: 'action', fixed: 'right', slots: { default: 'action' }, title: $t('common.operation'),
      minWidth: 250,
    },
  ],
  height: 500,
...
function selectedMaterialFun() {
  warehouseRecordWarehouseRecordStoreNumber(
    ...
    if (selectMaterialMessage.value.applyType === 18) {
      selectMaterialMessage.value.unit = $t('storesRequisition.unitPiece');
    }
    selectMaterialClose();
  });
}
...
function signAndIssue(bol = false) {
  const ids = getIds();
  if (ids.length > 0) {
    storeRequisitionSign(ids, bol).then(() => {
      message.success($t('common.successfulOperation'));
      gridApi.query();
    });
  }
...
function printFile() {
  ...
  if (accord) {
    ...
    if (semiFinishedProduct) {
      ...
      data = {
        time: dayjs().format($t('storesRequisition.dateFormat')),
    ...
    ob.then((res: any) => {
      try {
        ...
        console.error($t('storesRequisition.templateParseFailed'));
      }
    });
  } else {
    ...
    >
      ...
      <addOrUpdateGrid>
        ...
        <template #applyTypeName="{ row }">
          <Select
            v-model:value="row.applyType"
            class="!w-40"
            :options="listOfApplicationCategories"
            @change="
              (_label: any, option: any) => {
                row.applyType = option.value;
                row.applyTypeName = option.label;
              }
            "
            :disabled="isReadOnly"
          />
