...
function queryData({ page, pageSize }: any) {
  return new Promise((resolve, reject) => {
    ...
    // 调用 queryMeasureMethod 函数查询数据
    queryMeasureMethod(params)
      .then(({ total, results }) => {
        resolve({
          total,
          items: results,
...
function showEditDrawerFn(item?: any) {
  if (item) {
    ...
    queryMeasureMethodById(item.id).then((data) => {
      editItem.value = {
        ...data,
      };
    });
  } else {
    // 否则清空编辑对象，设置默认值，进行新增操作
    editItem.value = {
      isUse: 0,
      isTransit: 0,
    };
...
function submit() {
  editForm.value.validate().then(() => {
    ...
    // 根据是否有id判断是更新还是新增操作
    const ob = params.id
      ? updateMeasureMethod(params)
      : insertMeasureMethod(params);
    ob.then(() => {
      closeEditDrawer();
...
function delPhysicalWarehouse(row: any) {
  ...
  Modal.confirm({
    ...
    onOk() {
      // 调用删除接口
      deleteMeasureMethod(row.id).then(() => {
        message.success($t('common.successfulOperation'));
        // 刷新表格数据
        gridApi.reload();
      });
  ...
  <Page>
    ...
    >
      ...
      >
        ...
        <FormItem
          :label="
            $t('qualityModule.qualityCheck.qualityBaseSet.measureMaintain.name')
          "
          name="measureMethodName"
        >
          <Input v-model:value="editItem.measureMethodName" />
        </FormItem>
        <!-- 备注 -->
        <FormItem
          :label="
