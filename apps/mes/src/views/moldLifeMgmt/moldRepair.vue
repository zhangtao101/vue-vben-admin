 ...
 * [OUTPUT]: 对外提供模具报修单列表页面组件，含模具报修单列表查询、详情查看、取消功能
 * [POS]: 设备管理模块 的模具报修单列表页面
...
import MoldRepairRequestDrawer from '#/util/component/repairMaintenance/MoldRepairRequestDrawer.vue';
...
function initOptions() {
  searchBaseConfig({ configType: 'REPAIR_TYPE' }).then((res: any) => {
...
function handleConfirmCancel() {
  ...
  cancelRepairRequest(cancelRow.value.id, cancelReason.value)
    .then(() => {
  ...
  <Page>
    ...
    <Card class="!mb-4">
      <Form :model="queryParams" layout="inline">
        ...
        <!-- 模具名称 -->
        <FormItem
          :label="$t('repair.moldRepair.moldName')"
          style="margin-bottom: 0"
        >
          <Input
            v-model:value="queryParams.moldName"
            :placeholder="$t('repair.moldRepair.moldNamePlaceholder')"
            allow-clear
            style="width: 150px"
          />
        </FormItem>
