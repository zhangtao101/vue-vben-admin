...
const gridOptions: VxeGridProps<any> = {
  ...
  columns: [
    ...
    {
      field: 'manager',
      title: $t('riskManagement.planTaskResponsiblePerson'),
      minWidth: 150,
    },
    ...
    {
      field: 'startTime',
      title: $t('riskManagement.planCheckStartTime'),
      minWidth: 180,
    },
    {
      field: 'endTime',
      title: $t('riskManagement.planCheckEndTime'),
      minWidth: 180,
    },
    {
      field: 'calendarName',
    ...
    {
      field: 'cycle',
      title: $t('hiddenDangerInspectionPlan.inspectionPeriod'),
      minWidth: 150,
    },
...
function showEditFun(row?: any) {
  ...
  if (row) {
    queryPlanDetail(row.id).then((data) => {
      data.details = data.hazardCheckTypes;
      delete data.hazardCheckTypes;

      editItem.value = {
        ...data,
        riskName_riskType: `${row.riskName}&&${row.riskType}`,
      };

      editItem.value.details.forEach((item: any) => {
        queryInspectionType(item, true);
 ...
 * @param isInit 是否初始化
 */
function queryInspectionType(item: any, isInit?: boolean) {
  if (!isInit) {
    item.checkItem = '';
  }
  getInspectionTypeNamelist({
...
// region 风险评估标准
// 风险评估标准
const listOfEvaluationCriteria = ref<any>([]);

/**
 * 查询风险评估标准
 */
function queryEvaluationCriteria() {
  sourceList({
    pageNum: 1, // 当前页码。
    pageSize: 99_999, // 每页显示的数据条数。
  }).then(({ list }) => {
    listOfEvaluationCriteria.value = [];
    list.forEach((item: any) => {
      listOfEvaluationCriteria.value.push({
        label: item.riskName,
        value: `${item.riskName}&&${item.riskType}`,
      });
    });
  });
}
 ...
 * @param val
 */
function evaluationCriteriaChange(val: any) {
  const arr = val.split('&&');
  editItem.value.riskName = arr[0];
  editItem.value.riskType = arr[1];
}
// endregion
  ...
  <Page>
    ...
    >
      ...
      >
        ...
        >
          <Select
            v-model:value="editItem.riskName_riskType"
            :options="listOfEvaluationCriteria"
            show-search
            class="!w-56"
            :filter-option="filterOption"
        ...
        <template v-for="(item, index) of editItem.details" :key="index">
          ...
          >
            <Select
              ...
              @change="queryInspectionType(item)"
            />
          </FormItem>
