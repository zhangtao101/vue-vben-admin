 ...
 * SMT产品生产情况一览页面
 * 用于查看月度汇总和产品汇总
 * 功能包括：月度汇总查询、产品汇总查询、查看报工明细、导出数据
 */
...
function loadMonthData() {
  mouthList(listQuery)
    .then((data: any) => {
      ...
      result.node1 = '总计';
      monthGridApi.grid.reloadData([data.result]);
    })
    .catch((error: any) => {
 ...
 * 查询产品汇总
 */
function handleProductQuery({ page, pageSize }: any) {
  return new Promise((resolve, reject) => {
    ...
    poolList(params)
      .then(({ total, result, column }) => {
        ...
        productColumns.value = cols;
        // 更新列配置
        productGridApi.grid.loadColumn(cols);
  ...
  <Page content-class="SMTgeneral">
    ...
    <Card>
      <Form :model="listQuery" :rules="rules" layout="inline">
        ...
        <FormItem label="任务线别" class="!my-2">
          <Select
