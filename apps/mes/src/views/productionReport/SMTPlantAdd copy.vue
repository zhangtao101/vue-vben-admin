 ...
 * 使用 proxyConfig 代理加载，加载方式与主表格一致
 */
const manualGridOptions: VxeGridProps<any> = {
  align: 'center',
  border: true,
  columns: manualReportColumns,
  height: 300,
  pagerConfig: {
    enabled: true,
    pageSize: 20,
...
const [ManualGrid, manualGridApi] = useVbenVxeGrid({
  gridOptions: manualGridOptions,
});
...
/**
 * 刷新已挂载的明细表格
 * 未挂载的表格由 proxyConfig 首次渲染时自动加载
 */
function reloadDetailGrids() {
  if (readCodeGridApi?.grid) {
    readCodeGridApi.reload();
  }
  if (manualGridApi?.grid) {
    manualGridApi.reload();
  }
}
...
/**
 * 查询人工报工数据
 * 用于人工报工表格 proxyConfig 代理加载
 */
function queryManualData({ page, pageSize }: any) {
  return new Promise((resolve) => {
    if (!detailQuery.workstationCode || !detailQuery.bindingId) {
      resolve({ items: [], total: 0 });
      return;
