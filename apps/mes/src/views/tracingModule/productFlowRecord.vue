...
const gridOptions: VxeGridProps<any> = {
  ...
  columns: [
    ...
    {
      field: 'qrcode',
      title: $t('tracingModule.productTrace.qrcode'),
      minWidth: 150,
    },
    {
      field: 'processCode',
      title: $t('tracingModule.productFlowRecord.processCode'),
      minWidth: 100,
    },
    {
      field: 'processName',
    ...
    {
      field: 'equipCode',
      title: $t('tracingModule.productTrace.equipCode'),
      minWidth: 80,
    },
    {
      field: 'equipName',
    ...
    {
      field: 'productState',
      title: $t('tracingModule.productFlowRecord.productState'),
      minWidth: 80,
    },
    {
      field: 'opUser',
      title: $t('tracingModule.productFlowRecord.opUser'),
      minWidth: 80,
    },
    {
      field: 'action',
...
function handleDetail(row: any) {
  ...
  getProductDetailByParam(params)
    .then((resData: any) => {
      ...
      if (
        productState === '不良' ||
        productState === $t('tracingModule.productFlowRecord.defective')
      ) {
        drawerTitle.value = `${row.processCode}（${row.processName}）`;
        popData.value = {
  ...
  <Page>
    ...
    <Card class="!mb-4">
      <Form :model="{ inputQrcode }" layout="inline">
        <FormItem
          :label="$t('tracingModule.productTrace.qrcodeInput')"
          style="margin-bottom: 1em"
        >
          <Input
            v-model:value="inputQrcode"
            :placeholder="
              $t('tracingModule.productFlowRecord.qrcodePlaceholder')
            "
            allow-clear
            style="width: 250px"
      ...
      <template v-if="mainShow">
        ...
        >
          <Descriptions :column="3" bordered size="small">
            ...
            <Descriptions.Item
              :label="$t('tracingModule.productTrace.planCode')"
              :span="2"
            >
              {{ formData.planCode }}
            </Descriptions.Item>
            ...
            <Descriptions.Item
              :label="$t('tracingModule.productTrace.qrcode')"
              :span="3"
            >
              {{ formData.qrcode }}
            </Descriptions.Item>
    ...
    >
      <Spin :spinning="detailLoading">
        ...
        <template v-if="badDetailList.length > 0">
          <!-- 质检详情一览 -->
          <Card
            :title="$t('tracingModule.productFlowRecord.qcOverview')"
            size="small"
            class="!mb-4"
          >
            <Descriptions :column="2" bordered size="small">
              <Descriptions.Item
                :label="$t('tracingModule.productTrace.opTime')"
              >
                {{ popData.transferTime }}
              </Descriptions.Item>
              <Descriptions.Item
                :label="$t('tracingModule.productTrace.opUserName')"
              >
                {{ popData.opUser }}
              </Descriptions.Item>
              <Descriptions.Item
                :label="$t('tracingModule.productFlowRecord.judgmentResult')"
                :span="2"
              >
          ...
          <!-- 不良一览 -->
          <Card
            :title="$t('tracingModule.productTrace.defectList')"
            size="small"
          >
            <Descriptions :column="3" bordered size="small" class="!mb-4">
              <Descriptions.Item
                :label="$t('tracingModule.productTrace.workSheetCode')"
              >
                {{ popData.worksheetCode }}
              </Descriptions.Item>
              <Descriptions.Item
                :label="$t('tracingModule.productTrace.lineName')"
              >
                {{ popData.lineName }}
              </Descriptions.Item>
              <Descriptions.Item
                :label="$t('tracingModule.productTrace.defectNumber')"
              >
                {{ popData.defectNumber }}
              </Descriptions.Item>
            </Descriptions>

            <p class="mb-2 pl-2 font-semibold">
              {{ $t('tracingModule.productTrace.defectList') }}:
            </p>
            <Table
              :data-source="badDetailList"
              :columns="[
                {
                  title: $t('tracingModule.productTrace.defectCode'),
                  dataIndex: 'defectCode',
                  align: 'center',
                  width: 120,
                },
                {
                  title: $t('tracingModule.productTrace.defectName'),
                  dataIndex: 'defectName',
                  align: 'center',
                  width: 150,
                },
                {
                  title: $t('tracingModule.productTrace.defectNumber'),
                  dataIndex: 'defectNumber',
                  align: 'center',
                  width: 100,
                },
                {
                  title: $t('tracingModule.productTrace.location'),
                  dataIndex: 'location',
                  align: 'center',
                  width: 150,
                },
        ...
        <template
          v-if="
            drawerTitle === $t('tracingModule.productFlowRecord.repairDetail')
          "
        >
          <Card
            :title="$t('tracingModule.productFlowRecord.repairDetail')"
            size="small"
            class="!mb-4"
          >
            <Descriptions :column="2" bordered size="small">
              ...
              <Descriptions.Item
                :label="$t('tracingModule.productFlowRecord.thisNumber')"
              >
                {{ popData.number }}
              </Descriptions.Item>
              <Descriptions.Item
                :label="$t('tracingModule.productFlowRecord.repairTime')"
              >
                {{ popData.createTime }}
              </Descriptions.Item>
              <Descriptions.Item
                :label="$t('tracingModule.productFlowRecord.repairUser')"
              >
                {{ popData.repairUserName }}
              </Descriptions.Item>
              <Descriptions.Item
                :label="$t('tracingModule.productFlowRecord.repairResult')"
              >
                {{
                  popData.repairStatus === 1
                    ? $t('tracingModule.productFlowRecord.qualifed')
                    : $t('tracingModule.productFlowRecord.scrapped')
                }}
              </Descriptions.Item>
              <Descriptions.Item
                :label="$t('tracingModule.productTrace.defectCode')"
              >
                {{ popData.defectCode }}
              </Descriptions.Item>
            </Descriptions>
          ...
          <!-- 使用物料清单 -->
          <Card
            :title="$t('tracingModule.productFlowRecord.materialListUse')"
            size="small"
          >
            <Table
              ...
              :columns="[
                {
                  title: $t('tracingModule.productTrace.seq'),
                  align: 'center',
                  width: 60,
                },
                {
                ...
                {
                  title: $t('tracingModule.productFlowRecord.materialCount'),
                  dataIndex: 'number',
                  align: 'center',
                  width: 100,
                },
        ...
        <template
          v-if="
            drawerTitle === $t('tracingModule.productFlowRecord.packingDetail')
          "
        >
          <Card
            :title="$t('tracingModule.productFlowRecord.packingDetail')"
            size="small"
          >
            <Descriptions :column="2" bordered size="small">
              <Descriptions.Item
                :label="$t('tracingModule.productTrace.packingCode')"
              >
                {{ popData.packingCode }}
              </Descriptions.Item>
              <Descriptions.Item
                :label="$t('tracingModule.productTrace.packingType')"
              >
                {{ popData.stateName }}
              </Descriptions.Item>
              <Descriptions.Item
                :label="$t('tracingModule.productTrace.packingTime')"
              >
                {{ popData.opTime }}
              </Descriptions.Item>
              <Descriptions.Item
                :label="$t('tracingModule.productTrace.packingUser')"
              >
                {{ popData.opUserName }}
              </Descriptions.Item>
            </Descriptions>
