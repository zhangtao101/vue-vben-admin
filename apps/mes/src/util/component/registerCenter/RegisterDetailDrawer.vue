...
const FIELD_SCHEMA: Record<number, { columns: any[]; formFields: any[] }> = {
  ...
  1: {
    ...
    formFields: [
      ...
      {
        key: 'returnParamExample',
        label: 'returnParamExample',
        rows: 4,
        type: 'textarea',
      },
      {
...
const detailColumns = computed(() => {
  const schema = FIELD_SCHEMA[detailRow.value.registerType] ?? {
    columns: [],
    formFields: [],
  };
  const columns: any[] = [
    ...
    >
      ...
      <Descriptions :column="4" bordered class="mb-3">
        ...
        <DescriptionsItem :label="$t('functionRegisterManage.registerType')">
          <Tag
            :color="optionColor(registerTypeOptions, detailRow.registerType)"
          >
        ...
        <DescriptionsItem :label="$t('functionRegisterManage.registerFlag')">
          <Tag
            :color="optionColor(registerFlagOptions, detailRow.registerFlag)"
          >
        ...
        <DescriptionsItem :label="$t('functionRegisterManage.registerTrigger')">
          <Tag
            :color="
              optionColor(registerTriggerOptions, detailRow.registerTrigger)
            "
          >
      ...
      <DetailGrid>
        ...
        <template #action="{ row }">
          ...
          <Tooltip>
            ...
            <Button class="mr-4" type="link" @click="openEdit('view', row)">
              <Icon
    ...
    >
      ...
      <template #footer>
        <Space>
          ...
          <!-- 确认 -->
          <Button v-if="currentMode !== 'view'" type="primary" @click="submit">
            {{ $t('common.confirm') }}
          </Button>
        </Space>
      </template>
