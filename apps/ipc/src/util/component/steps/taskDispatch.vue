 ...
 * 任务下发组件
 * @description
 * 用于 AGV 加料任务演示，展示当前液位、任务触发警戒值以及任务清单列表，
 * 支持对任务执行"任务下发"操作（纯前端演示，无接口调用）。
 * 页面从上到下布局：
 * 1. 当前液位和任务触发警戒值显示
 * 2. 任务清单列表（序号、任务号、任务类型、AGV小车id、吨桶ID、起始点位、目标点位/目标机台、小车运行状态、任务状态、操作）
 * 3. 操作列提供"任务下发"按钮
 * @since 2026-08-19 09:00:00
 */
...
/**
 * 当前液位（百分比）
 * 用于演示任务触发警戒值判断，取值 0-100
 */
const liquidLevel = ref(65);
/**
 * 任务触发警戒值（百分比）
 * 当液位低于该值时触发加料任务
 */
const warnValue = ref(30);

/**
 * 是否达到任务触发条件
 * 功能：根据当前液位与警戒值判断是否需要触发加料任务
 *
 * @returns {boolean} 当前液位低于或等于警戒值时返回 true
 *
 * 注意事项：
 * - 演示数据，后续接入真实数据源时需替换 liquidLevel/warnValue 来源
 */
function isWarnLevel() {
  return liquidLevel.value <= warnValue.value;
}
...
<template>
  <div>
    <!-- 液位与警戒值展示区 -->
    <div
      class="mb-4 grid grid-cols-1 gap-4 rounded-lg border border-gray-200 p-4 md:grid-cols-2"
    >
      <!-- 当前液位 -->
      <div class="flex items-center justify-between rounded-md bg-blue-50 p-4">
        <span class="text-base font-medium">当前液位</span>
        <span
          :class="
            isWarnLevel() ? 'text-xl font-bold text-red-500' : 'text-xl font-bold text-blue-600'
          "
        >
          {{ liquidLevel }}%
        </span>
      ...
      <!-- 任务触发警戒值 -->
      <div class="flex items-center justify-between rounded-md bg-orange-50 p-4">
        <span class="text-base font-medium">任务触发警戒值</span>
        <span class="text-xl font-bold text-orange-600">{{ warnValue }}%</span>
      </div>
      <!-- 液位预警提示 -->
      <div v-if="isWarnLevel()" class="md:col-span-2">
        <Tag color="red">液位已低于警戒值，请及时下发加料任务！</Tag>
      </div>
      <div v-else class="md:col-span-2">
        <Tag color="green">液位正常，暂无需下发加料任务</Tag>
      </div>
    </div>
