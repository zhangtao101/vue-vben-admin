/** * 运输监控组件 * @description * 用于 AGV
运输监控演示，页面采用左右两栏布局： * -
左侧：运输任务清单表格，字段包括任务号、AGV小车ID、吨桶ID、起始点位、目标机台、任务状态、操作（监控查看按钮），并通过表格行颜色区分任务状态（未执行-白色、执行中-黄色、已完成-绿色）。
* - 右侧：Canvas 绘制的现场布局图（库位网格 + AGV 小车 +
路径箭头），根据左侧表格选中的任务显示对应 AGV 位置。 * 纯前端演示，无接口调用。
...
function drawScene() {
  ...
  if (selectedTask.value) {
    drawSelectedMarker(
      ...
      w,
      h,
      selectedTask.value.agvX,
      selectedTask.value.agvY,
    );
  }
}
  ...
  <div class="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_1fr]">
    ...
    >
      ...
      >
        ...
        <div>
          状态：<Tag
            :color="getTaskStatusColor(selectedTask.taskStatus)"
            class="ml-1"
          >
