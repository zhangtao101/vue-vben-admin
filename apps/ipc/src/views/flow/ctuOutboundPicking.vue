...
/**
 * CTU出库拣货组件
 * 用于制造执行系统中的CTU(Carton Transfer Unit)出库拣货作业管理
 * 主要功能：开始拣货、完成拣货、物料拣货操作、物料特征选择
 * 支持两种模式：有标签模式和无标签模式
 * 在有标签模式下，通过扫描标签自动获取物料信息
 * 在无标签模式下，需要手动选择物料和特征
 * 与入库拣货的区别：使用productPackingOut接口进行出库操作，界面采用左右分栏布局
 */

// region 表格信息配置
// VXE表格配置对象，定义了出库拣货详情表格的结构和行为
const gridOptions: VxeGridProps<any> = {
  ...
  columns: [
    ...
    { field: 'number', title: '数量', minWidth: 150 }, // 出库拣货数量
...
/**
 * 开始出库拣货操作
 * 调用拣货开始接口，成功后显示左侧表单区域（此组件采用左右分栏布局）
 * 如果接口调用失败，则隐藏表单区域
 */
function start() {
  productPackingStart({
...
// region 出库拣货操作相关状态和方法

// 出库拣货操作表单区域的显示状态控制（此组件使用左右分栏布局而非抽屉）
const isShow = ref(false);

// 出库拣货操作表单的引用，用于表单验证
const formRef = ref();

// 出库拣货操作编辑信息对象，存储当前拣货记录的所有字段数据
 ...
 */
function openDrawer() {
  isShow.value = true; // 显示表单区域（触发左右分栏布局）
...
function closeDrawer() {
  isShow.value = false; // 隐藏表单区域（恢复整栏显示）
}

/**
 * 出库拣货操作提交
 * 验证表单数据后调用出库拣货接口，成功后刷新数据
 */
...
<!--
  CTU出库拣货组件模板
  包含查询表单区域、数据表格区域、左右分栏的拣货操作表单和特征选择抽屉
  采用左右分栏布局：表格在左侧，操作表单在右侧（与入库拣货的抽屉布局不同）
-->
<template>
  <Page>
    <!-- 主页面卡片容器，显示出库拣货相关操作 -->
    <Card :title="$t('ctuCartonPicking.packingAndPicking')">
      ...
      <!-- 左右分栏布局区域 -->
      <Row>
        <!-- 左侧表格栏：根据表单显示状态动态调整宽度 -->
        <Col :span="isShow ? 12 : 24">
          <Grid />
        </Col>
        <!-- 右侧表单栏：仅在开始拣货后显示，占用12列宽度 -->
        <Col :span="12" v-if="isShow">
          <!-- 出库拣货操作表单，包含所有拣货相关的输入字段 -->
          <Form
            ref="formRef"
          ...
          >
            ...
            <FormItem :wrapper-col="{ span: 16, offset: 8 }">
              ...
              <!-- 取消按钮：隐藏表单区域且不保存数据 -->
              <Button @click="closeDrawer()" class="my-2 w-full">
                {{ $t('common.cancel') }}
              </Button>
            </FormItem>
          </Form>
