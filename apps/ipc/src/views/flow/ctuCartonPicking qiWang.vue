...
const gridOptions: VxeGridProps<any> = {
  ...
  border: true, // 显示表格边框
  columns: [
    { field: 'materialCode', title: $t('ctuCartonPicking.productCode'), minWidth: 150 }, // 物料的唯一编码
    { field: 'materialName', title: $t('ctuCartonPicking.productName'), minWidth: 150 }, // 物料的描述性名称
...
const queryMaterialCodeAndName = debounce((materialCode: string) => {
  ...
  getByProductCodeAndName({
    productCode: materialCode, // 传入查询参数
  })
    .then(({ list: results }: any) => {
      // 格式化物料显示标签，组合物料名称和编码
...
function materialCodeChange(_value: any, item: any) {
  editedInformation.value.materialName = item.productName; // 设置物料名称
  editedInformation.value.unit = item.unit; // 设置物料单位
  queryCharacteristics(item.productCode); // 查询该物料的特征信息
}
  ...
  <Page>
    ...
    >
      ...
      >
        ...
        <!-- 产品编号选择：根据标签模式显示不同控件 -->
        <FormItem
          :label="$t('ctuCartonPicking.productCode')"
          :rules="[{ required: true, message: '该项为必填项' }]"
          name="materialCode"
        >
          <!-- 有标签模式：显示只读输入框 -->
        ...
        <!-- 产品名称显示（只读） -->
        <FormItem :label="$t('ctuCartonPicking.productName')">
          <Input v-model:value="editedInformation.materialName" disabled />
        </FormItem>
        <!-- 物料特征选择区域（已注释停用：如需恢复请移除 v-if="false"） -->
        <FormItem
          v-if="false"
